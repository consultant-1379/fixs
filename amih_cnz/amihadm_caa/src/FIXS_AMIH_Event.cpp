...
//
FIXS_AMIH_Event::FIXS_AMIH_Event()
{
}
FIXS_AMIH_Event::~FIXS_AMIH_Event()
{
...
FIXS_AMIH_Event* FIXS_AMIH_Event::instance()
{
    return instance_::instance();
}


FIXS_AMIH_EventHandle FIXS_AMIH_Event::CreateEvent(bool manualReset, bool initialState, const char* name)
{
    FIXS_AMIH_EventHandle eventHandle = -1;
    ...
    if ((eventHandle = eventfd(0, 0)) == -1) {
        ...
        return FIXS_AMIH_Event_RC_ERROR;
    }
...
FIXS_AMIH_EventHandle FIXS_AMIH_Event::OpenNamedEvent(string name)
{
    ...
    FIXS_AMIH_EventHandle handle = it->second->handle;
    instance()->lock_.release();
...
void FIXS_AMIH_Event::CloseEvent(FIXS_AMIH_EventHandle eventHandle)
{
    instance()->lock_.acquire_write();
...
int FIXS_AMIH_Event::WaitForEvents(int count, int *eventHandles, int timeoutInMilliseconds)
{
    fd_set fds;
    int nfds = 0;
    ...
    if (ret == 0) {
        ...
        return FIXS_AMIH_Event_RC_TIMEOUT;
    ...
    if (n != -1)
    {
        ...
        FIXS_AMIH_Event::EventProperties_t *ep = NULL;

        // locate what type of event this is, start in the named events list
        instance()->lock_.acquire_read();
        FIXS_AMIH_Event::NamedEventIteratorType neit = instance()->findNamedEventByHandle(handleActivated);
        instance()->lock_.release();
        ...
        if (ep == NULL) {
            ...
            instance()->lock_.acquire_read();
            FIXS_AMIH_Event::EventIteratorType ueit = instance()->findEventByHandle(handleActivated);
            instance()->lock_.release();
...
bool FIXS_AMIH_Event::SetEvent(FIXS_AMIH_EventHandle eventHandle)
{
    ssize_t sz;
    uint64_t n = 1;
    ...
    if (sz != sizeof(uint64_t)) {
        // write fault
        //ACS_CS_TRACE((FIXS_AMIH_Event_TRACE, "(%t) Failed to set event for handle: %d", eventHandle));
        return false;
    }
...
bool FIXS_AMIH_Event::ResetEvent(FIXS_AMIH_EventHandle eventHandle)
{
    fd_set fds;
    struct timeval tv;
...
FIXS_AMIH_Event::NamedEventIteratorType FIXS_AMIH_Event::findNamedEventByHandle(FIXS_AMIH_EventHandle handle)
{
    // check the list with named events
...
// NOTE! Must be wrapped inside acquire/release statement
FIXS_AMIH_Event::EventIteratorType FIXS_AMIH_Event::findEventByHandle(FIXS_AMIH_EventHandle handle)
{
    // check the list of unnamed events
