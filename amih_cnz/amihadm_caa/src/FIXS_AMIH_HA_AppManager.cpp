...
FIXS_AMIH_HA_AppManager::FIXS_AMIH_HA_AppManager(const char* daemon_name):ACS_APGCC_ApplicationManager(daemon_name){
...
FIXS_AMIH_HA_AppManager::~FIXS_AMIH_HA_AppManager(){
	//destroy dynamic allocations

}


ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performStateTransitionToActiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState){
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performStateTransitionToPassiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState) {
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performStateTransitionToQueisingJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performStateTransitionToQuiescedJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
        (void)previousHAState;
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performComponentHealthCheck(void)
{
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performComponentTerminateJobs(void)
{
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performComponentRemoveJobs(void)
{
...
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::performApplicationShutdownJobs() {
...
ACE_THR_FUNC_RETURN svc_run(void *ptr){
	FIXS_AMIH_HA_AppManager *haObj = (FIXS_AMIH_HA_AppManager*) ptr;
        haObj->svc();
        return 0;
}
ACS_APGCC_ReturnType FIXS_AMIH_HA_AppManager::svc(){
        ...
        while(true)
        {
                ...
                if (fds[0].revents & POLLIN){
                        ...
                        if (ha_state[0] == 'A'){
                                ...
                                if (startSRV == NULL){
                                    startSRV = new (std::nothrow) FIXS_AMIH_Server();
                                	startSRV->run();
                                }
                        }
                        ...
                        if (ha_state[0] == 'S'){
                        	...
                        	FIXS_AMIH_EventHandle handle = FIXS_AMIH_Event::OpenNamedEvent(AMIH_Util::EVENT_NAME_SHUTDOWN);
                        	if (handle >= 0)
                        		(void) FIXS_AMIH_Event::SetEvent(handle);

                        	if(startSRV){
                        		sleep(AMIH_Util::Daemon_Timeout);
                        		delete startSRV;
                        		startSRV = NULL;
                        	}
                        	if (handle >= 0)
                        		FIXS_AMIH_Event::ResetEvent(handle);
