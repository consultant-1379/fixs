/*
 * FIXS_CCH_ApManagedSw.h
 *
 *  Created on: Apr 20, 2012
 *      Author: eanform
 */

#ifndef FIXS_CCH_APMANAGEDSW_H_
#define FIXS_CCH_APMANAGEDSW_H_
...
class FIXS_CCH_ApManagedSw: public acs_apgcc_objectimplementerinterface_V3 {
public:
	...
	FIXS_CCH_ApManagedSw();

	virtual ~FIXS_CCH_ApManagedSw();
...
private:
	...
	void addObjectCcbId(ACS_APGCC_CcbId ccbId, std::string dn, CCH_Util::operationCallBackType opt, std::string container="", int fbn=0,  std::string attrib="");

	void removeObjectCcbId(std::string dn);

	void cleanCcbMap();

	IMM_Util::ActionSwResultInfo checkMagAndSlotParameters(ACS_APGCC_AdminOperationParamType **paramList, std::string &mag, std::string &slot, std::string product);

	IMM_Util::ActionSwResultInfo checkAllParameters(ACS_APGCC_AdminOperationParamType **paramList, std::string &mag, std::string &slot, IMM_Util::BoardUpgradeType  &upgradeType, bool &force, std::string product);


	typedef std::map<std::string, FIXS_CCH_ApManagedSw::CcbIdMapData> mapObjectCcb;
	...
	FIXS_CCH_ThreadActions* ApActionThread;

	ACS_TRA_Logging *FIXS_CCH_logging;
};


#endif /* FIXS_CCH_APMANAGEDSW_H_ */
