...
FIXS_CCH_Event* FIXS_CCH_Event::instance()
{
    return instance_::instance();
}


FIXS_CCH_EventHandle FIXS_CCH_Event::CreateEvent(bool manualReset, bool initialState, const char* name)
{
    FIXS_CCH_EventHandle eventHandle = -1;
    ...
    if ((eventHandle = eventfd(0, 0)) == -1) {
        ...
        return FIXS_CCH_Event_RC_ERROR;
    }
...
FIXS_CCH_EventHandle FIXS_CCH_Event::OpenNamedEvent(string name)
{
    ...
    FIXS_CCH_EventHandle handle = it->second->handle;
    instance()->lock_.release();
...
void FIXS_CCH_Event::CloseEvent(FIXS_CCH_EventHandle eventHandle)
{
    instance()->lock_.acquire_write();
...
int FIXS_CCH_Event::WaitForEvents(int count, int *eventHandles, int timeoutInMilliseconds)
{
    fd_set fds;
    int nfds = 0;
    ...
    if (ret == 0) {
        ...
        return FIXS_CCH_Event_RC_TIMEOUT;
    ...
    if (n != -1)
    {
        ...
        FIXS_CCH_Event::EventProperties_t *ep = NULL;

        // locate what type of event this is, start in the named events list
        instance()->lock_.acquire_read();
        FIXS_CCH_Event::NamedEventIteratorType neit = instance()->findNamedEventByHandle(handleActivated);
        instance()->lock_.release();
        ...
        if (ep == NULL) {
            ...
            instance()->lock_.acquire_read();
            FIXS_CCH_Event::EventIteratorType ueit = instance()->findEventByHandle(handleActivated);
            instance()->lock_.release();
...
bool FIXS_CCH_Event::SetEvent(FIXS_CCH_EventHandle eventHandle)
{
    ssize_t sz;
    uint64_t n = 1;
    ...
    if (sz != sizeof(uint64_t)) {
        // write fault
        //ACS_CS_TRACE((FIXS_CCH_Event_TRACE, "(%t) Failed to set event for handle: %d", eventHandle));
        return false;
    }
...
bool FIXS_CCH_Event::ResetEvent(FIXS_CCH_EventHandle eventHandle)
{
    fd_set fds;
    struct timeval tv;
...
FIXS_CCH_Event::NamedEventIteratorType FIXS_CCH_Event::findNamedEventByHandle(FIXS_CCH_EventHandle handle)
{
    // check the list with named events
...
// NOTE! Must be wrapped inside acquire/release statement
FIXS_CCH_Event::EventIteratorType FIXS_CCH_Event::findEventByHandle(FIXS_CCH_EventHandle handle)
{
    // check the list of unnamed events
