...
FIXS_CCH_HA_AppManager::FIXS_CCH_HA_AppManager(const char* daemon_name):ACS_APGCC_ApplicationManager(daemon_name)
{

	/* create the pipe for shutdown handler */
	Is_terminated = FALSE;

	startSRV = NULL; //TO CHECK ???
...
/*	// create the pipe for shutdown handler
	Is_terminated = FALSE;
	startSRV = NULL;
	//startSRV = new (std::nothrow) FIXS_CCH_Server();
...
FIXS_CCH_HA_AppManager::~FIXS_CCH_HA_AppManager()
{
	FIXS_CCH_logging = 0;
}

ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performStateTransitionToActiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performStateTransitionToPassiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performStateTransitionToQueisingJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performStateTransitionToQuiescedJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	(void)previousHAState;
	ACE_TCHAR state[1] = {'S'};
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performComponentHealthCheck(void)
{
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performComponentTerminateJobs(void)
{
	...
	syslog(LOG_INFO, "Application Assignment is Terminate now");
	/* Inform the thread to go "stop" state */
	if ( !Is_terminated ) {
		write(readWritePipe[1], &state, sizeof(state));
		syslog(LOG_INFO, "Waiting for the application thread to terminate.");
	...
	syslog(LOG_INFO, "Application is Terminated");
	return ACS_APGCC_SUCCESS;
}

ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performComponentRemoveJobs(void)
{
...
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::performApplicationShutdownJobs()
{
...
ACE_THR_FUNC_RETURN svc_run(void *ptr){
	FIXS_CCH_HA_AppManager *haObj = (FIXS_CCH_HA_AppManager*) ptr;
        haObj->svc();
        return 0;
}
ACS_APGCC_ReturnType FIXS_CCH_HA_AppManager::svc()
{
	struct pollfd fds[1];
		nfds_t nfds = 1;
		ACE_INT32 ret;
		...
		syslog(LOG_INFO, "Starting Application Thread");
		 if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("Starting Application Thread",LOG_LEVEL_DEBUG);
...
//				syslog(LOG_INFO, "timeout on ACE_OS::poll");
			...
			if (fds[0].revents & POLLIN){
				...
				if (ha_state[0] == 'A'){
					...
					if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("Thread:: Application is Active",LOG_LEVEL_DEBUG);
					/* start application work */
					//FIXS_CCH_Server startSRV; // NO
					if (startSRV == NULL){
						startSRV = new (std::nothrow) FIXS_CCH_Server();
						startSRV->run();
					}
				...
				if (ha_state[0] == 'P'){
					...
					if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("Thread:: Application is Passive",LOG_LEVEL_DEBUG);
					/* do nothing */
				}
				...
				if (ha_state[0] == 'S'){
					...
					if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("Thread:: Request to stop application",LOG_LEVEL_DEBUG);
					/* Request to stop the thread, perform the gracefull activities here */

					FIXS_CCH_EventHandle handle = FIXS_CCH_Event::OpenNamedEvent(CCH_Util::EVENT_NAME_SHUTDOWN);
					if (handle >=0)(void) FIXS_CCH_Event::SetEvent(handle);

					if(startSRV){
						sleep(CCH_Util::Daemon_Timeout);
						delete startSRV;
						startSRV = NULL;
					}
					if (handle >=0) FIXS_CCH_Event::ResetEvent(handle);
					break;
				}
			}
		}
