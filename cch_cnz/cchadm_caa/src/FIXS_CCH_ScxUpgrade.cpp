...
struct Lock
{
	Lock()
	{
		//std::cout << "FIXS_CCH_ScxUpgrade: Lock region while exception active." << std::endl;
		ACE_OS::thread_mutex_trylock(&s_cs);
	};

	~Lock()
	{
		//std::cout << "FIXS_CCH_ScxUpgrade: Leave Lock region while exception active." << std::endl;
...
struct Lock_snmp
{
	Lock_snmp()
	{
		//std::cout << "FIXS_CCH_ScxUpgrade: SNMP Lock region while exception active." << std::endl;
		ACE_OS::thread_mutex_trylock(&s_cs_snmp);
	};

	~Lock_snmp()
	{
		//std::cout << "FIXS_CCH_ScxUpgrade: SNMP Leave Lock region while exception active." << std::endl;
		ACE_OS::thread_mutex_unlock(&s_cs_snmp);
	};
};
}
...
namespace {
	...
	const std::string INSTALLATION_FOLDER = "/data/apz/data/boot/SCX/";
...
FIXS_CCH_ScxUpgrade::ScxState::ScxState()
      : m_context(0)
{
	m_id = FIXS_CCH_ScxUpgrade::STATE_UNDEFINED;
	m_set_status = false;
}

FIXS_CCH_ScxUpgrade::ScxState::ScxState (FIXS_CCH_ScxUpgrade::ScxStateId stateId)
      : m_context(0)
{
...
//## Other Operations (implementation)
 void FIXS_CCH_ScxUpgrade::ScxState::setContext (FIXS_CCH_ScxUpgrade* context)
{
 	m_context = context;
}

FIXS_CCH_ScxUpgrade::ScxStateId FIXS_CCH_ScxUpgrade::ScxState::getStateId () const
{
 	return m_id;
}

 int FIXS_CCH_ScxUpgrade::ScxState::activateSW ()
{
 ...
 int FIXS_CCH_ScxUpgrade::ScxState::commit ()
{
	int result = 0;
	//std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << std::endl;
	std::cout << " *********************** WRONG EVENT IN THIS STATE" <<std::endl;
 ...
 int FIXS_CCH_ScxUpgrade::ScxState::prepare (std::string packageToUpgrade, std::string productToUpgrade)
{
 ...
 int FIXS_CCH_ScxUpgrade::ScxState::cancel ()
{
	int result = 0;
	//std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << std::endl;
	std::cout << " *********************** WRONG EVENT IN THIS STATE" <<std::endl;
 ...
 int FIXS_CCH_ScxUpgrade::ScxState::loadReleaseCompleted (int loadResult)
{
 	int result = 0;
	//std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << std::endl;
 ...
 int FIXS_CCH_ScxUpgrade::ScxState::coldStart ()
{
 	int result = 0;
	//std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << std::endl;
	std::cout << " *********************** WRONG EVENT IN THIS STATE" <<std::endl;
...
FIXS_CCH_ScxUpgrade::State_Activated::State_Activated()
 :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_ACTIVATED)
{
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Activated::commit ()
{
	...
	if (success)
	{
		...
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_COMMIT_RECEIVED);
	}
	else
	{
		...
		//try once again
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_COMMIT_RECEIVED_FAILED);
 ...
 int FIXS_CCH_ScxUpgrade::State_Activated::cancel ()
{
	int result = 0;
	...
	else
	{
		...
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_CANCEL_RECEIVED);
		result = EXECUTED;
	}
...
FIXS_CCH_ScxUpgrade::State_Activating::State_Activating()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_ACTIVATING)
{
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Activating::coldStart ()
{
	int res = 0;
	...
	sleep(1); //SCX needs some second to update the MIB
	...
	if (readSuccess)
	{
		...
		sleep(1);
		//activated (new value!)
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_WARMRESTART_NEW_COMPLETED);
		res = EXECUTED;
	}
	else
	{
		...
		//in order to cold restart SCX we need to change the state machine by handling a new state logic to 
		...
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_WARMRESTART_OLD_COMPLETED);
...
FIXS_CCH_ScxUpgrade::State_Automatic_Rollback_Ended::State_Automatic_Rollback_Ended()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_AUTOMATIC_ROLLBACK_ENDED)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Automatic_Rollback_Ended::commit ()
{
  	int result = 0;
...
FIXS_CCH_ScxUpgrade::State_Manual_Rollback_Ended::State_Manual_Rollback_Ended()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_MANUAL_ROLLBACK_ENDED)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Manual_Rollback_Ended::commit ()
{
  	int result = 0;
...
FIXS_CCH_ScxUpgrade::State_Idle::State_Idle()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_IDLE)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Idle::prepare (std::string packageToUpgrade, std::string productToUpgrade)
{
  	/*
	...
	*/
...

	{
		...
		if(m_context->FIXS_CCH_logging)  m_context->FIXS_CCH_logging->Write(traceChar,LOG_LEVEL_WARN);
	...
	{
		result = m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_PREPARE_RECEIVED);
		result = EXECUTED;
	}
...
FIXS_CCH_ScxUpgrade::State_Preparing::State_Preparing()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_PREPARING)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Preparing::cancel ()
{
	int result = 0;
 ...
 int FIXS_CCH_ScxUpgrade::State_Preparing::loadReleaseCompleted (int loadResult)
{
  	int result = 0;
	...
	{
		//ok
		m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_PREPARE_OK);
		result = EXECUTED;
	...
	else
	{
		...
		else
		{
			...
			m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_FAILURE_RECEIVED);
...
FIXS_CCH_ScxUpgrade::State_Prepared::State_Prepared()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_PREPARED)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Prepared::cancel ()
{
	int result = 0;
 ...
 int FIXS_CCH_ScxUpgrade::State_Prepared::activateSW ()
{
	...
	else if (m_context->startRelease())
	{
		...
		m_context->maintenanceWindow(FIXS_CCH_JTPHandler::IMBSW_START_UPGRADE);
		result = m_context->setEvent(FIXS_CCH_ScxUpgrade::SCX_ACTIVATE_RECEIVED);
	}
	else
	{
		result = SNMP_FAILURE; //13
...
FIXS_CCH_ScxUpgrade::State_Manual_Rollback_Ongoing::State_Manual_Rollback_Ongoing()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_MANUAL_ROLLBACK_ONGOING)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Manual_Rollback_Ongoing::coldStart ()
{
...
FIXS_CCH_ScxUpgrade::State_Failed::State_Failed()
  :FIXS_CCH_ScxUpgrade::ScxState(FIXS_CCH_ScxUpgrade::STATE_FAILED)
{
}
...
//## Other Operations (implementation)
 int FIXS_CCH_ScxUpgrade::State_Failed::prepare (std::string packageToUpgrade, std::string productToUpgrade)
{
  	bool success = false;
	int result = 0;
...
FIXS_CCH_ScxUpgrade::EventQueue::EventQueue()
     : m_queueHandle(-1)
{
	//Lock lock;
...
//## Other Operations (implementation)
FIXS_CCH_ScxUpgrade::ScxEventId FIXS_CCH_ScxUpgrade::EventQueue::getFirst ()
{
	//FIXS_CCH_CriticalSectionGuard eventLock(m_queueCriticalSection);
	//Lock lock;

	ScxEventId result = SCX_DEFAULT_NO_EVENT;
 ...
 void FIXS_CCH_ScxUpgrade::EventQueue::popData ()
{
//	 FIXS_CCH_CriticalSectionGuard eventLock(m_queueCriticalSection);
	 //Lock lock;
 ...
 void FIXS_CCH_ScxUpgrade::EventQueue::pushData (FIXS_CCH_ScxUpgrade::ScxEventId info)
{
//	 FIXS_CCH_CriticalSectionGuard eventLock(m_queueCriticalSection);
	 //Lock lock;

	 {
		 ...
		 {
			 //check first element
			 FIXS_CCH_ScxUpgrade::ScxEventId first;
			 first = getFirst();
 ...
 bool FIXS_CCH_ScxUpgrade::EventQueue::queueEmpty ()
{
	 //Lock lock;
 ...
 size_t FIXS_CCH_ScxUpgrade::EventQueue::queueSize ()
{
	 //Lock lock;
...
FIXS_CCH_ScxUpgrade::ScxEventId FIXS_CCH_ScxUpgrade::EventQueue::getItem (int index)
{
	...
	if (it == m_qEventInfo.end())
		return SCX_DEFAULT_NO_EVENT;
	else
		return *it;

}

void FIXS_CCH_ScxUpgrade::EventQueue::cleanQueue()
{
	while (!m_qEventInfo.empty()) {
		m_qEventInfo.pop_front();
	}
...
FIXS_CCH_ScxUpgrade::FIXS_CCH_ScxUpgrade()
      : trapTimer(NULL),
        m_exit(false),
        m_running(false),
        m_lastErrorCode(0),
...
FIXS_CCH_ScxUpgrade::FIXS_CCH_ScxUpgrade (unsigned short boardId, unsigned long magazine, unsigned short slot, std::string installedPackage, std::string scx_IP_EthA, std::string scx_IP_EthB, std::string dn)
	:trapTimer(NULL), m_exit(false), m_running(false),
	 m_lastErrorCode(0),
	 m_upgradeStatus(0),
	 m_fullUpradeType(false),
	 m_eventQueue(),
	 m_slot(slot), m_magazine(magazine), m_boardId(boardId), m_scx_IP_EthA(scx_IP_EthA), m_scx_IP_EthB(scx_IP_EthB), m_currentCxp(installedPackage), m_expectedCxp(""),m_Object_DN(dn)
	 ,m_active_commit(0)
{
	...
	m_scx_IP = m_scx_IP_EthA;

	traceObj = new ACS_TRA_trace("FIXS_CCH_ScxUpgrade");
...
//	}
 ...
 int FIXS_CCH_ScxUpgrade::activateSW ()
{
...
//  	std::cout << "DBG:####################### " << __FUNCTION__ << "@" << __LINE__ << " m_currentState->get() " << getStateName().c_str()<< std::endl;
 ...
 int FIXS_CCH_ScxUpgrade::commit ()
{
...
int FIXS_CCH_ScxUpgrade::prepare (std::string packageToUpgrade, std::string productToUpgrade)
{
 ...
 int FIXS_CCH_ScxUpgrade::setEvent (FIXS_CCH_ScxUpgrade::ScxEventId event)
{
 ...
 int FIXS_CCH_ScxUpgrade::open (void *args)
 {
	 int result = 0;
	 UNUSED(args);
	 ...
	 {//log
		 char traceChar[512] = {0};
		 snprintf(traceChar, sizeof(traceChar) - 1,"SCX UPGRADE Starting thread on instance [%p]. \n",this);
...
//	const int Stop_Event_Index=3;
	...
	while ( ! m_exit ) // Set to true when the thread shall exit
	{
		...
		case WAIT_FAILED:   // Event failed
			...
			{
				//std::cout << __FUNCTION__ << "@" << __LINE__ << " ScxUpgrade is waiting for events..." << std::endl;
				if (traceObj->ACS_TRA_ON())
				{
					char tmpStr[512] = {0};
			...
			{
				std::cout << __FUNCTION__ << "@" << __LINE__ << " ScxUpgrade Shutdown_Event_Index..." << std::endl;
				if (traceObj->ACS_TRA_ON())
				{
					char tmpStr[512] = {0};
		...
		case (WAIT_OBJECT_0 + Command_Event_Index):    // Command ready to be handled
			{
				//std::cout << __FUNCTION__ << "@" << __LINE__ << " ScxUpgrade Command_Event_Index => handleCommandEvent()" << std::endl;
				printMachineInfo(m_magazine, m_slot, getStateName());

				if (traceObj->ACS_TRA_ON())
				{
					char tmpStr[512] = {0};
					snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ScxUpgrade Command_Event_Index => handleCommandEvent()", __FUNCTION__, __LINE__);
					traceObj->ACS_TRA_event(1,tmpStr);
					if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_DEBUG);
				}
...
//				if (m_eventQueue.queueEmpty())
		...
		case (WAIT_OBJECT_0 + (TimeoutTrap_Event_Index)):
			{
				std::cout << __FUNCTION__ << "@" << __LINE__ << " ScxUpgrade TimeoutTrap_Event_Index => handleTrapTimeout()" << std::endl;

				if (traceObj->ACS_TRA_ON())
				{
					char tmpStr[512] = {0};
					snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ScxUpgrade TimeoutTrap_Event_Index on event:[%d]", __FUNCTION__, __LINE__,m_timeoutTrapEvent);
					traceObj->ACS_TRA_event(1,tmpStr);
					if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_DEBUG);
				}
...
//		{
//			std::cout << __FUNCTION__ << "@" << __LINE__ << " ScxUpgrade Stop_Event_Index" << std::endl;
//
//
//			{
//				char tmpStr[512] = {0};
//				snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ScxUpgrade Stop_Event_Index", __FUNCTION__, __LINE__);
//				if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,tmpStr);
//				if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_DEBUG);
//			}
//
...
//		}
	...
	{
		char tmpStr[512] = {0};
		snprintf(tmpStr, sizeof(tmpStr) - 1, "FIXS_CCH_ScxUpgrade [%s@%d] Exiting from while", __FUNCTION__, __LINE__);
		if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_DEBUG);
		if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,tmpStr);
	}
 ...
 void FIXS_CCH_ScxUpgrade::stop ()
 {
	 m_exit = true;

	 {//log
		 char traceChar[512] = {0};
		 snprintf(traceChar, sizeof(traceChar) - 1,"SCX UPGRADE Stopping thread on instance [%p]. \n",this);
		 if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(traceChar,LOG_LEVEL_DEBUG);
		 if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,traceChar);
	 }
...
//	 }
 ...
 void FIXS_CCH_ScxUpgrade::handleCommandEvent ()
 {
	 ...
	 while (!m_eventQueue.queueEmpty()) {
		 ...
		 FIXS_CCH_ScxUpgrade::ScxEventId qEventInfo = SCX_DEFAULT_NO_EVENT;
...
void FIXS_CCH_ScxUpgrade::handleTrapTimeout ()
{

	std::string expectedSoftware("SCX_");
	...
	else if (getStateName() == "ACTIVATING") // timer on cold start
	{
		...
		else
		{
			...
			setEvent(FIXS_CCH_ScxUpgrade::SCX_WARMRESTART_OLD_COMPLETED);
	...
	else if (getStateName() == "MANUAL_ROLLBACK_ONGOING") // timer on cold start
	{
		...
		else
		{
			...
			//version didn't rollback
			setEvent(FIXS_CCH_ScxUpgrade::SCX_FAILURE_RECEIVED);
		}
 ...
 bool FIXS_CCH_ScxUpgrade::cancelTrapTimer ()
{
 ...
 bool FIXS_CCH_ScxUpgrade::startTrapTimer (int seconds)
{
...
//		return false;
 ...
 void FIXS_CCH_ScxUpgrade::initOnStart ()
{
...
//		m_StopEvent=FIXS_CCH_Event::OpenNamedEvent(eventNameStop);
 ...
 void FIXS_CCH_ScxUpgrade::switchState (FIXS_CCH_ScxUpgrade::ScxEventId eventId)
{
...
//	int lastReason = 0;
	...
	switch(eventId)
	{
		case SCX_PREPARE_RECEIVED:
			t_event = "SCX_PREPARE_RECEIVED";
			break;

		case SCX_ACTIVATE_RECEIVED:
			t_event = "SCX_ACTIVATE_RECEIVED";
			break;

		case SCX_COMMIT_RECEIVED:
			t_event = "SCX_COMMIT_RECEIVED";
			break;

		case SCX_CANCEL_RECEIVED:
			t_event = "SCX_CANCEL_RECEIVED";
			break;

		case SCX_PREPARE_OK:
			t_event = "SCX_PREPARE_OK";
			break;
		
		case SCX_WARMRESTART_NEW_COMPLETED:
			t_event = "SCX_WARMRESTART_NEW_COMPLETED";
			break;

		case SCX_WARMRESTART_OLD_COMPLETED:
			t_event = "SCX_WARMRESTART_OLD_COMPLETED";
			break;
		
		case SCX_COMMIT_OK:
			t_event = "SCX_COMMIT_OK";
			break;
		
		case SCX_FAILURE_RECEIVED:
			t_event = "SCX_FAILURE_RECEIVED";
			break;

		case SCX_ROLLBACK_OK:
			t_event = "SCX_ROLLBACK_OK";
			break;

		case SCX_DEFAULT_NO_EVENT:
			t_event = "SCX_DEFAULT_NO_EVENT";
			break;
	...
	switch(m_currentState->getStateId())
	{
		...
		case STATE_IDLE:
			switch(eventId)
			{
				case SCX_PREPARE_RECEIVED: // IDLE, SCX_PREPARE_RECEIVED == > PREPARING
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " IDLE: SCX_PREPARE_RECEIVED == > PREPARING "<<std::endl;

					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] IDLE: SCX_PREPARE_RECEIVED == > PREPARING ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_INFO);
					}
		...
		case STATE_PREPARING:
			switch(eventId)
			{
				case SCX_PREPARE_OK: // PREPARING: SCX_PREPARE_OK == > PREPARED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " PREPARING: SCX_PREPARE_OK == > PREPARED"<<std::endl;
					
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] PREPARING: SCX_PREPARE_OK == > PREPARED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
					...
					for ( int i=0; i<5; i++)
					{
					   if(modifySwInventory(status))
						   break;
					}
					// TR HZ51036 END
					break;

				case SCX_CANCEL_RECEIVED: // PREPARING: SCX_CANCEL_RECEIVED == > MANUAL_ROLLBACK_ENDED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " PREPARING: SCX_CANCEL_RECEIVED == > MANUAL_ROLLBACK_ENDED "<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] PREPARING: SCX_CANCEL_RECEIVED == > MANUAL_ROLLBACK_ENDED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
				...
				case SCX_FAILURE_RECEIVED: // PREPARING: SCX_FAILURE_RECEIVED == > AUTOMATIC_ROLLBACK_ENDED
					
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " PREPARING: SCX_FAILURE_RECEIVED == > AUTOMATIC_ROLLBACK_ENDED "<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] PREPARING: SCX_FAILURE_RECEIVED == > AUTOMATIC_ROLLBACK_ENDED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
					cancelTrapTimer();
		...
		case STATE_PREPARED:
			switch(eventId)
			{
				case SCX_ACTIVATE_RECEIVED: // PREPARED: SCX_ACTIVATE_RECEIVED == > ACTIVATING
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " PREPARED: SCX_ACTIVATE_RECEIVED == > ACTIVATING"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] PREPARED: SCX_ACTIVATE_RECEIVED == > ACTIVATING ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
				...
				case SCX_CANCEL_RECEIVED: // PREPARED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ENDED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " PREPARED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ENDED"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] PREPARED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ENDED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
		...
		case STATE_ACTIVATING:
			switch(eventId)
			{
				case SCX_WARMRESTART_NEW_COMPLETED: // ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > ACTIVATED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > ACTIVATED"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > ACTIVATED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_INFO);
					}
				...
				case SCX_WARMRESTART_OLD_COMPLETED: // ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > AUTOMATIC_ROLLBACK_ONGOING
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > AUTOMATIC_ROLLBACK_Ended "<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ACTIVATING: SCX_WARMRESTART_NEW_COMPLETED == > AUTOMATIC_ROLLBACK_Ended ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_INFO);
					}
		...
		case STATE_ACTIVATED:
			switch(eventId)
			{
				case SCX_COMMIT_RECEIVED: // ACTIVATED: SCX_COMMIT_RECEIVED == > IDLE
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " ACTIVATED: SCX_COMMIT_RECEIVED == > IDLE"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ACTIVATED: SCX_COMMIT_RECEIVED == > IDLE ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
				...
				case SCX_COMMIT_RECEIVED_FAILED: // ACTIVATED: SCX_COMMIT_RECEIVED_FAILED == > SW_ERR_COMMIT_ERROR
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " ACTIVATED: SCX_COMMIT_RECEIVED_FAILED == > SW_ERR_COMMIT_ERROR"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ACTIVATED: SCX_COMMIT_RECEIVED_FAILED == > SW_ERR_COMMIT_ERROR ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
				...
				case SCX_CANCEL_RECEIVED: // ACTIVATED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ONGOING
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " ACTIVATED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ONGOING"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] ACTIVATED: SCX_CANCEL_RECEIVED == > MANUAL ROLL BACK ONGOING ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);
					}
		...
		case STATE_MANUAL_ROLLBACK_ONGOING:
			switch(eventId)
			{
				case SCX_WARMRESTART_OLD_COMPLETED: // MANUAL_ROLLBAK_ONGOING: SCX_ROLLBACK_OK == > MANUAL_ROLLBAK_ENDED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " MANUAL_ROLLBAK_ONGOING: SCX_ROLLBACK_OK == > MANUAL_ROLLBAK_ENDED"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] MANUAL_ROLLBAK_ONGOING: SCX_ROLLBACK_OK == > MANUAL_ROLLBAK_ENDED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_INFO);
					}
				...
				case SCX_FAILURE_RECEIVED: // MANUAL_ROLLBAK_ONGOING: SCX_FAILURE_RECEIVED == > FAILED
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " MANUAL_ROLLBAK_ONGOING: SCX_FAILURE_RECEIVED == > FAILED"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] MANUAL_ROLLBAK_ONGOING: SCX_FAILURE_RECEIVED == > FAILED ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_ERROR);
					}
...
//					m_currentState = &m_failed;
		...
		case STATE_MANUAL_ROLLBACK_ENDED:
			switch(eventId)
			{
				case SCX_COMMIT_RECEIVED: // MANUAL_ROLLBAK_END: SCX_COMMIT_RECEIVED == > IDLE
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " MANUAL_ROLLBACK_END: SCX_COMMIT_RECEIVED == > IDLE"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] MANUAL_ROLLBACK_END: SCX_COMMIT_RECEIVED == > IDLE ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_INFO);
					}
					m_currentState = &m_idle;
...
//					status = CCH_Util::SW_ERR_OK;
		...
		case STATE_AUTOMATIC_ROLLBACK_ENDED:
			switch(eventId)
			{
				case SCX_COMMIT_RECEIVED: // AUTOMATIC_ROLLBAK_END: SCX_COMMIT_RECEIVED == > IDLE
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " AUTOMATIC_ROLLBAK_END: SCX_COMMIT_RECEIVED == > IDLE"<<std::endl;
					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] AUTOMATIC_ROLLBAK_END: SCX_COMMIT_RECEIVED == > IDLE ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_DEBUG);
					}
					m_currentState = &m_idle;
...
//					status = CCH_Util::SW_ERR_OK;
		...
		case STATE_FAILED:
			switch(eventId)
			{
				case SCX_PREPARE_RECEIVED: // FAILED, SCX_PREPARE_RECEIVED == > PREPARING
					std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " FAILED: SCX_PREPARE_RECEIVED == > PREPARING "<<std::endl;

					if (traceObj->ACS_TRA_ON())
					{
						char tmpStr[512] = {0};
						snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] FAILED: SCX_PREPARE_RECEIVED == > PREPARING ", __FUNCTION__, __LINE__);
						traceObj->ACS_TRA_event(1,tmpStr);
						if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_ERROR);
					}
 ...
 int FIXS_CCH_ScxUpgrade::cancel ()
{
//
//	 std::cout << "DBG:####################### " << __FUNCTION__ << "@" << __LINE__ << " m_currentState->get() " << getStateName().c_str()<< std::endl;
	 if (strcmp(m_SwInventory_Object_DN.c_str(),"") == 0 )
 ...
 std::string FIXS_CCH_ScxUpgrade::getStateName ()
{
	std::string t_currState="";
 ...
 bool FIXS_CCH_ScxUpgrade::configureDHCP (bool rollback)
{
...
//	CCH_Util::findAndReplace(magazine,".","_");
	...
	cout << "---------------------- API END---------------------------------------- " << endl;
        { 
        char tmpStr[512] = {0};
        snprintf(tmpStr, sizeof(tmpStr) - 1, "[%s@%d] magazine: %s, slot: %d, m_currentCxp: %s, m_currentCxc: %s, getExpectedCXP: %s, getExpectedCXC: %s", __FUNCTION__, __LINE__, magazine.c_str(),m_slot,m_currentCxp.c_str(),m_currentCxc.c_str(),getExpectedCXP().c_str(),getExpectedCXC().c_str());
                if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,tmpStr);
                if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(tmpStr,LOG_LEVEL_WARN);	 
        }
	...
	if(rollback)
		...
		IMM_Interface::updateUsedPackage(m_magazine,m_slot,getExpectedCXP(),m_currentCxp);	
                if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("Rollback, updated with m_currentCXP",LOG_LEVEL_WARN);
         }
	else
         {
		IMM_Interface::updateUsedPackage(m_magazine,m_slot,m_previousCxp,getExpectedCXP());
                if(FIXS_CCH_logging)  FIXS_CCH_logging->Write("updated with getExceptedCXP",LOG_LEVEL_WARN);
	 }
	switch (result)
	{
 ...
 bool FIXS_CCH_ScxUpgrade::preliminaryCheckOnPrepare (std::string softwareVersion, std::string containerId)
{

	bool result = true;
	std::string currCXC("");
	currCXC = ACS_APGCC::after(this->m_currentCxc,"SCX_");
 ...
 FIXS_CCH_ScxUpgrade::ScxStateId FIXS_CCH_ScxUpgrade::getStateId () const
{
  	return m_currentState->getStateId();
}

 bool FIXS_CCH_ScxUpgrade::loadRelease ()
{
	...
	do  // if the first snmp statement fails, trying second one
	{
		...
		//url = "tftp://192.168."+ sub + ".33/SCX/"+ getExpectedCXP();
		
		std::string folder = INSTALLATION_FOLDER + getExpectedCXP();
		
		//todo
		scxfilename=getScxFileName(m_expectedCxp);

//		scxfilename = "SCX3.tar";
		
		//url = "tftp://192.168."+ sub + ".33/SCX/"+ getExpectedCXP() + "/" + scxfilename;

		if (m_APNode.length() <= 0)
		{
			//Old method
			url = "tftp://192.168."+ sub + ".33/boot/SCX/"+ getExpectedCXP() + "/" + scxfilename;
		}
		else
		{
			if (m_APNode == "A")
			{
				url = "tftp://192.168."+ sub + ".1/boot/SCX/"+ getExpectedCXP() + "/" + scxfilename;
			}
			else if (m_APNode == "B")
			{
				url = "tftp://192.168."+ sub + ".2/boot/SCX/"+ getExpectedCXP() + "/" + scxfilename;
			}
			else
			{
				if (traceObj->ACS_TRA_ON())
	...
	if (result)
	{
		...
		m_snmpManager->setIpAddress(m_scx_IP_EthA);
 ...
 int FIXS_CCH_ScxUpgrade::loadReleaseCompleted (int loadResult)
{
 	int res=0;
 ...
 bool FIXS_CCH_ScxUpgrade::checkLoadedVersion ()
{
 ...
 bool FIXS_CCH_ScxUpgrade::checkCurrentLoadedVersion ()
{
	...
	int countFlag=0;
	string ipadd("");
	std::string productExpected("SCX_");
 ...
 int FIXS_CCH_ScxUpgrade::coldStart ()
{
	int res=0;
//	std::cout << "DBG:####################### " << __FUNCTION__ << "@" << __LINE__ << " m_currentState->get() " << getStateName().c_str()<< std::endl;
	{ //log
 ...
 void FIXS_CCH_ScxUpgrade::maintenanceWindow (unsigned char msgType)
 {
 ...
 bool FIXS_CCH_ScxUpgrade::readActionStatus ()
{
 ...
 bool FIXS_CCH_ScxUpgrade::startRelease ()
{
	...
	do  // if the first snmp statement fails, trying second one
	{
		std::string version = "SCX_" + getExpectedCXC();
 ...
 bool FIXS_CCH_ScxUpgrade::makeDefault ()
{
 ...
 void FIXS_CCH_ScxUpgrade::resetSwitch ()
 {

	 FIXS_CCH_SNMPManager * m_snmpManager = NULL;

	 // reset switch can only be issued on the SCX Master because
	 // the reset OID belongs to the IPMI MIB

	 int countFlag = 0;
	 m_snmpManager = new FIXS_CCH_SNMPManager(m_scx_IP_EthA);
	 std::string ipAddress = m_scx_IP_EthA;

	 do{
		 int resMaster = m_snmpManager->isMaster(ipAddress);
		 if (resMaster == 1)//master
		 {
			 m_snmpManager->resetSwitch(m_slot); // cold restart SCXB
			 { //trace
...

				 {
					 ...
					 unsigned char u_plug0 = (unsigned char)(hlmag >> 24);
					 unsigned char mag = u_plug0 & 0x0F;
					 snprintf(traceChar, sizeof(traceChar) - 1, "[%s@%d][MAG:%x, SLOT:%u] Cold restart SCXB through resetSwitch", __FUNCTION__, __LINE__, mag, m_slot);
					 if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,traceChar);
					 if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(traceChar,LOG_LEVEL_DEBUG);
				 }
		 ...
		 {
			 std::string m_neighborSCXIP = m_neighborSCXIPA;
			 m_snmpManager->setIpAddress(m_neighborSCXIPA);

			 //issue the reset on the master SCX
			 if (!m_snmpManager->resetSwitch(m_slot))
			 {
				 m_snmpManager->setIpAddress(m_neighborSCXIPB);
				 m_neighborSCXIP = m_neighborSCXIPB;
			 ...
			 { //trace

				 {
					 ...
					 unsigned char u_plug0 = (unsigned char)(hlmag >> 24);
					 unsigned char mag = u_plug0 & 0x0F;
					 snprintf(traceChar, sizeof(traceChar) - 1, "[%s@%d][MAG:%x, SLOT:%u] This is a passive SCX, the resetSwitch is going through neighbor address %s",
							 __FUNCTION__, __LINE__, mag, m_slot, m_neighborSCXIP.c_str());
					 if (traceObj->ACS_TRA_ON())traceObj->ACS_TRA_event(1,traceChar);
					 if(FIXS_CCH_logging)  FIXS_CCH_logging->Write(traceChar,LOG_LEVEL_DEBUG);
				 }
 ...
 void FIXS_CCH_ScxUpgrade::saveStatus ()
{
 ...
 void FIXS_CCH_ScxUpgrade::onRestart ()
{
	...
	if (getStateName() == "PREPARING" )
	{
		...
		if (checkResult)
		{
			// success
			std::cout << "DBG: " << __FUNCTION__ << "@" << __LINE__ << " checkLoadedVersion(" << expectedSoftware.c_str() << ") SUCCESS!!!" << std::endl;
			setEvent(FIXS_CCH_ScxUpgrade::SCX_PREPARE_OK);
		}
		else
		{
			// failed
...
//		if (m_jtpHandler[1]) m_jtpHandler[1]->open();
		...
		if (checkCurrentLoadedVersion())
		{
			...
			setEvent(FIXS_CCH_ScxUpgrade::SCX_WARMRESTART_NEW_COMPLETED);
		}
		else
		{
			startTrapTimer(TEN_MINUTES);
...
//		if (m_jtpHandler[1]) m_jtpHandler[1]->open();
 ...
 void FIXS_CCH_ScxUpgrade::getOverallStatus (CCH_Util::SWData &swdata)
{
 ...
 void FIXS_CCH_ScxUpgrade::restoreStatus (CCH_Util::SWData &swdata)
{
 ...
 std::string FIXS_CCH_ScxUpgrade::getCurrentRunningSoftware ()
{
 ...
 bool FIXS_CCH_ScxUpgrade::readResultStatus ()
{
 ...
 bool FIXS_CCH_ScxUpgrade::checkRollBackVersion ()
{
	...
	while (!exit)
	{
		if (m_snmpManager->getCurrentLoadedVersion(scxVer))
		{
			std::cout << __FUNCTION__ << "@" << __LINE__ << 
			" - checkRollBackVersion: " << scxVer << 
			" - expectedSoftware: " << expectedSoftware << std::endl;
			
			exit = true;
				if (scxVer.compare(expectedSoftware) != 0 )
				{
					//true
					result = true;
 ...
 bool FIXS_CCH_ScxUpgrade::checkCurrentDefaultVersion ()
{
	
	FIXS_CCH_SNMPManager * m_snmpManager = NULL;

	std::string currLoadVer("");
	std::string currentSoftware("SCX_");
 ...
 bool FIXS_CCH_ScxUpgrade::restoreMakeDefault ()
 {
 ...
 void FIXS_CCH_ScxUpgrade::setObjectDN(const char* dn)
 {
	 m_Object_DN = dn;
 }

 void FIXS_CCH_ScxUpgrade::setSwInvObjectDN(std::string dn)
 {
	 //m_SwInventory_Object_DN = dn.c_str();
	 m_SwInventory_Object_DN = dn;
 }

 std::string FIXS_CCH_ScxUpgrade::getSwInvObjectDN()
 {
	 return m_SwInventory_Object_DN;
 }

 std::string FIXS_CCH_ScxUpgrade::getSwPercistanceDN()
 {
	 return dn_blade_persistance;
 }

 bool FIXS_CCH_ScxUpgrade::modifySwInventory(int state)
 {
	 int lastResult = 0;
	 int lastReason = 0;
	 std::string lastDate("");
	 ...
	 currSw = ACS_APGCC::after(m_currentCxc,"SCX_");
 ...
 bool FIXS_CCH_ScxUpgrade::initSwInventory()
 {
	 int currState = this->getStateId();
 ...
 int FIXS_CCH_ScxUpgrade::getStateImm ()
 {
	 int t_currState = -1;
 	switch(getStateId())
 	{
 ...
 std::string  FIXS_CCH_ScxUpgrade::getScxFileName( std::string package)
 {
	 std::string fileName("");
...
void FIXS_CCH_ScxUpgrade::resetEvents ()
{
	FIXS_CCH_Event::ResetEvent(m_cmdEvent);
	FIXS_CCH_Event::ResetEvent(m_timeoutTrapEvent);
...
bool FIXS_CCH_ScxUpgrade::sendStartRequestToCP (bool & shutdownsignaled)
{
...
void FIXS_CCH_ScxUpgrade::WaitJtpNotificationRetriesToFinish()
{
