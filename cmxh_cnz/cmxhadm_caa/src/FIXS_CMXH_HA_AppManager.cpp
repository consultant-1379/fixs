...
FIXS_CMXH_HA_AppManager::FIXS_CMXH_HA_AppManager(const char* daemon_name):ACS_APGCC_ApplicationManager(daemon_name)
{
...
/*	// create the pipe for shutdown handler
	Is_terminated = FALSE;
	startSRV = NULL;
	//startSRV = new (std::nothrow) FIXS_CMXH_Server();
...
FIXS_CMXH_HA_AppManager::~FIXS_CMXH_HA_AppManager()
{
	FIXS_CMXH_logging = 0;
}

ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performStateTransitionToActiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performStateTransitionToPassiveJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performStateTransitionToQueisingJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performStateTransitionToQuiescedJobs(ACS_APGCC_AMF_HA_StateT previousHAState)
{
	(void)previousHAState;
	ACE_TCHAR state[1] = {'S'};
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performComponentHealthCheck(void)
{
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performComponentTerminateJobs(void)
{
	...
	syslog(LOG_INFO, "My Application Component is terminated !!!");
	return ACS_APGCC_SUCCESS;
	// HX64016 END
}

ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performComponentRemoveJobs(void)
{
...
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::performApplicationShutdownJobs()
{
...
ACE_THR_FUNC_RETURN svc_run(void *ptr){
	FIXS_CMXH_HA_AppManager *haObj = (FIXS_CMXH_HA_AppManager*) ptr;
        haObj->svc();
        return 0;
}
ACS_APGCC_ReturnType FIXS_CMXH_HA_AppManager::svc()
{
	struct pollfd fds[1];
		nfds_t nfds = 1;
		ACE_INT32 ret;
		...
		if(FIXS_CMXH_logging)  FIXS_CMXH_logging->Write("Starting Application Thread",LOG_LEVEL_DEBUG);
...
//				syslog(LOG_INFO, "timeout on ACE_OS::poll");
			...
			if (fds[0].revents & POLLIN){
				...
				if (ha_state[0] == 'A'){
					...
					if(FIXS_CMXH_logging)  FIXS_CMXH_logging->Write("Thread:: Application is Active",LOG_LEVEL_DEBUG);
					/* start application work */
					//FIXS_CMXH_Server startSRV; // NO
					if (startSRV == NULL){
						startSRV = new (std::nothrow) FIXS_CMXH_Server();
						startSRV->run();
					}
				...
				if (ha_state[0] == 'P'){
					...
					if(FIXS_CMXH_logging)  FIXS_CMXH_logging->Write("Thread:: Application is Passive",LOG_LEVEL_DEBUG);
					/* do nothing */
				}
				...
				if (ha_state[0] == 'S'){
					 ...
					 */
					if(FIXS_CMXH_logging)  FIXS_CMXH_logging->Write("Thread:: Request to stop application",LOG_LEVEL_DEBUG);
					/* Request to stop the thread, perform the gracefull activities here */

					FIXS_CMXH_EventHandle handle = FIXS_CMXH_Event::OpenNamedEvent(CMXH_Util::EVENT_NAME_SHUTDOWN);
					if (handle >=0)(void) FIXS_CMXH_Event::SetEvent(handle);
					...
					if (handle >=0) FIXS_CMXH_Event::ResetEvent(handle);
					break;
				}
			}
		}
