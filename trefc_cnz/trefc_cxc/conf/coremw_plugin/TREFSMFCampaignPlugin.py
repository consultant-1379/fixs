...
def createSMFCampaignPlugin():
   return TREFSMFCampaignPlugin()

class TREFSMFCampaignPlugin(SMFCampaignPlugin):
   # initialization method
   # self: variable represents the instance of the object itself
   def __init__(self):
      super(TREFSMFCampaignPlugin, self).__init__()
      self.MY_COMPONENT_UID = "fixs.trefc"
      self._info = None
      self._utils = None
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
   def cliAtCampInit(self):
      ...
      trefc = self._info.getComponent("fixs.trefc")
      for bundle in trefc.swBundles:
         bundleName = bundle[1] 
      result = []
      ...
      if (self._actionType == SMFConstants.CT_INSTALL or self._actionType == SMFConstants.CT_UPGRADE or self._actionType == SMFConstants.CT_MIGRATE):
         cli = os.path.join("$OSAFCAMPAIGNROOT", self.MY_COMPONENT_UID, "scripts", "trefc_helper.sh" )
