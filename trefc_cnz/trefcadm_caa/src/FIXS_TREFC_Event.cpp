...
//
FIXS_TREFC_Event::FIXS_TREFC_Event()
{
}
FIXS_TREFC_Event::~FIXS_TREFC_Event()
{
...
FIXS_TREFC_Event* FIXS_TREFC_Event::instance()
{
    return instance_::instance();
}


FIXS_TREFC_EventHandle FIXS_TREFC_Event::CreateEvent(bool manualReset, bool initialState, const char* name)
{
    FIXS_TREFC_EventHandle eventHandle = -1;
    ...
    if ((eventHandle = eventfd(0, 0)) == -1) {
        ...
        return FIXS_TREFC_Event_RC_ERROR;
    }
...
FIXS_TREFC_EventHandle FIXS_TREFC_Event::OpenNamedEvent(string name)
{
    ...
    FIXS_TREFC_EventHandle handle = it->second->handle;
    instance()->lock_.release();
...
void FIXS_TREFC_Event::CloseEvent(FIXS_TREFC_EventHandle eventHandle)
{
    instance()->lock_.acquire_write();
...
int FIXS_TREFC_Event::WaitForEvents(int count, int *eventHandles, int timeoutInMilliseconds)
{
    fd_set fds;
    int nfds = 0;
    ...
    if (ret == 0) {
        ...
        return FIXS_TREFC_Event_RC_TIMEOUT;
    ...
    if (n != -1)
    {
        ...
        FIXS_TREFC_Event::EventProperties_t *ep = NULL;

        // locate what type of event this is, start in the named events list
        instance()->lock_.acquire_read();
        FIXS_TREFC_Event::NamedEventIteratorType neit = instance()->findNamedEventByHandle(handleActivated);
        instance()->lock_.release();
        ...
        if (ep == NULL) {
            ...
            instance()->lock_.acquire_read();
            FIXS_TREFC_Event::EventIteratorType ueit = instance()->findEventByHandle(handleActivated);
            instance()->lock_.release();
...
bool FIXS_TREFC_Event::SetEvent(FIXS_TREFC_EventHandle eventHandle)
{
    ssize_t sz;
    uint64_t n = 1;
    ...
    if (sz != sizeof(uint64_t)) {
        // write fault
        //ACS_CS_TRACE((FIXS_TREFC_Event_TRACE, "(%t) Failed to set event for handle: %d", eventHandle));
        return false;
    }
...
bool FIXS_TREFC_Event::ResetEvent(FIXS_TREFC_EventHandle eventHandle)
{
    fd_set fds;
    struct timeval tv;
...
FIXS_TREFC_Event::NamedEventIteratorType FIXS_TREFC_Event::findNamedEventByHandle(FIXS_TREFC_EventHandle handle)
{
    // check the list with named events
...
// NOTE! Must be wrapped inside acquire/release statement
FIXS_TREFC_Event::EventIteratorType FIXS_TREFC_Event::findEventByHandle(FIXS_TREFC_EventHandle handle)
{
    // check the list of unnamed events
